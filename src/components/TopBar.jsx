...
import { ShieldCheck, Wifi, Bell, Radio } from 'lucide-react';
...
export default function TopBar({ clock, level, tab, setTab, alertCount, onlineCount, totalCount, demoMode, simTime, decisionStage }) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '100%', padding: '0 16px', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ fontSize: 15, fontWeight: 900, color: '#E8F4FD', letterSpacing: '-0.3px' }}>WadiGuard</span>
              <span style={{
            ...
            <p style={{ fontSize: 9, color: '#6B7D93', lineHeight: 1.2, marginTop: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          <span style={{
            ...
            color: demoMode === 'jury' ? '#00F0FF' : demoMode === 'live' ? '#F59E0B' : '#22C55E',
          }}>
            {demoMode === 'jury' ? '🎯 DÉMO JURY' : demoMode === 'live' ? '▶ SIMULATION' : '● NOMINAL'}
          </span>
                ...
                return (
                  <div key={c} style={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <div title={CHAIN_FULL[i]} style={{
                      ...
                      border: `1px solid ${active ? color : done ? 'rgba(0,240,255,0.2)' : '#1E2530'}`,
                      color: active ? color : done ? '#00F0FF' : '#4A5568',
                      transition: 'all 0.3s ease',
                    }}>{c}</div>
                    {i < CHAIN.length - 1 && (
                      <span style={{ fontSize: 7, color: done ? '#00F0FF' : '#1E2530', margin: '0 1px' }}>›</span>
                    )}
                  </div>
                );
              })}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
          ...
          <div style={{
            ...
            <div>
              <p style={{ fontSize: 8, textTransform: 'uppercase', letterSpacing: '0.15em', color: '#6B7D93', lineHeight: 1.2 }}>Niveau</p>
              <p style={{ fontSize: 11, fontWeight: 800, color, lineHeight: 1.2 }}>{meta.label}</p>
            </div>
          </div>

          {/* Sensors + Alerts */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 10.5, color: '#6B7D93' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
              <Wifi size={11} color="#22C55E" /> {onlineCount}/{totalCount}
            </span>
            {alertCount > 0 && (
              <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                <Bell size={11} color="#F59E0B" />
                <span style={{ color: '#F59E0B', fontWeight: 700 }}>{alertCount}</span>
              </span>
            )}
          </div>
          ...
          <div style={{ textAlign: 'right', minWidth: 60 }}>
            {demoMode === 'jury' && (
              <p className="mono-precision" style={{ fontSize: 14, fontWeight: 800, color: '#00F0FF', lineHeight: 1.1 }}>
                T+{String(Math.round(simTime || 0)).padStart(2, '0')}s
              </p>
            )}
            <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: demoMode === 'jury' ? 10 : 14, fontWeight: 700, color: '#E8F4FD', lineHeight: 1.2 }}>{hms}</p>
            <p style={{ fontSize: 9, color: '#4A5568' }}>{date}</p>
          </div>
        </div>
      </div>
    </header>
