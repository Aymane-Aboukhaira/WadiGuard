  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 0, width: '100%', overflow: 'hidden' }}>
      {DECISION_STAGES.map((stage, i) => {
        ...
        const nodeColor = isActive ? '#00F0FF' : isPast ? '#22C55E' : 'var(--wg-border)';
        const textColor = isActive ? '#00F0FF' : isPast ? '#22C55E' : 'var(--wg-muted)';
        ...
        return (
          <React.Fragment key={stage}>
            ...
            {i > 0 && (
              <div style={{
                flex: '1 1 0', height: 1, minWidth: 4,
                background: isPast || isActive ? '#22C55E' : 'var(--wg-border)',
            ...
            <div style={{
              ...
              <div style={{
                ...
                background: isActive ? 'rgba(0,240,255,0.12)' : isPast ? 'rgba(34,197,94,0.08)' : 'rgba(255,255,255,0.02)',
                border: `1.5px solid ${nodeColor}`,
                boxShadow: isActive ? '0 0 10px rgba(0,240,255,0.3)' : 'none',
                transition: 'all 0.4s ease',
              }}>
                <Icon size={isActive ? 11 : 9} color={nodeColor} />
              </div>
              <span style={{
